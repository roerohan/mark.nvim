...
local function validate_config(cfg)
  ...
  if cfg.split_position and
     not vim.tbl_contains({'right', 'left', 'top', 'bottom'}, cfg.split_position) then
    return false, 'split_position must be one of: right, left, top, bottom'
  end

  if cfg.split_size and
     (type(cfg.split_size) ~= 'number' or cfg.split_size < 1 or cfg.split_size > 100) then
